...
AmazonMwsClient.prototype.call = function(api, action, query) {
	...
	//qs.stringify(sorted, { allowDots: true })
	...
	//This fixes an issue signing requests due to the nested object
	//TODO: Add this code to a loop so it can handle multiple fee requests
	if(requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1"]){
		var obj = requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1"];
        requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1.MarketplaceId"] = obj.MarketplaceId;
        requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1.IdType"] = obj.IdType;
        requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1.IdValue"] = obj.IdValue;
        requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1.IsAmazonFulfilled"] = obj.IsAmazonFulfilled;
        requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1.Identifier"] = obj.Identifier;
        requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1.PriceToEstimateFees.ListingPrice.Amount"] = obj.PriceToEstimateFees.ListingPrice.Amount;
        requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1.PriceToEstimateFees.ListingPrice.CurrencyCode"] = obj.PriceToEstimateFees.ListingPrice.CurrencyCode;
        delete requestOpts.form["FeesEstimateRequestList.FeesEstimateRequest.1"];
	}

	return new Promise(function(resolve, reject) {
		request(requestOpts, function(err, response, data){
            mwsRequestAction(err, response, data, resolve, reject, requestOpts);
		});
	});
};

function mwsRequestAction(err, response, data, resolve, reject, requestOpts, waitMills){
    if (err) {
        reject(err);
    } else {
        if(data.slice(0, 5) !== '<?xml' && response.headers["content-type"] === "text/xml"){
            //Update response with xml format
            ...
            parser.parseString(data, function(err, result) {
                ...
                if (err != null) {
                    ...
                    if(result.ErrorResponse && result.ErrorResponse.Error &&
                        result.ErrorResponse.Error.Code === "RequestThrottled"){
                        //Request throttled. Wait and try again
						console.log("Request Throttled. Waiting...");
						//Defaut waitMills to 3 sec
						if(!waitMills)
							waitMills = 10000;
						else //Double wait time
                            waitMills = waitMills * 2;
						setTimeout(function () {
                            request(requestOpts, function(err, response, data){
                                mwsRequestAction(err, response, data, resolve, reject, requestOpts, waitMills);
                            });
                        }, waitMills);
                    }
...
AmazonMwsClient.prototype.sign = function(path, query) {
	...
	var stringToSign = ["POST", this.host, path, qs.stringify(sorted, { allowDots: true })].join("\n");
	//var stringToSign = "POST\nmws.amazonservices.co.uk\n/\nAWSAccessKeyId=AKIAJL3JUOQNIJWB3CCQ&Action=GetReportList&MaxCount=1&Merchant=A1XSOPJZUY0ABI&ReportTypeList.Type.1=_GET_FLAT_FILE_OPEN_LISTINGS_DATA_&SignatureMethod=HmacSHA256&SignatureVersion=2&Timestamp=2017-09-03T23%3A42%3A29Z&Version=2009-01-01";

	var sha256Hmac = CryptoJS.HmacSHA256(stringToSign, this.secretAccessKey);
	var base64 = CryptoJS.enc.Base64.stringify(sha256Hmac);
